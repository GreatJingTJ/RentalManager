  ...
  void dialogEditInCatergory(name, imageURL) {
    ...
    void submit() async {
      ...
      if (form.validate()) {
        ...
        var  a = await Firestore.instance.collection(returnItemCollection()).where('Location', isEqualTo: LocationName).where('category', isEqualTo: name).getDocuments();
        var b = await Firestore.instance.collection(returnReservationCollection()).where('location', isEqualTo: LocationName).where('category', isEqualTo: name).getDocuments();
        var docList = a.documents;
        var docList2 = b.documents;

        for(int i = 0; i < docList.length; i++){
          await Firestore.instance.collection(returnItemCollection()).document(docList[i].documentID).updateData({
        ...
        for(int i = 0; i < docList2.length; i++){
          await Firestore.instance.collection(returnReservationCollection()).document(docList2[i].documentID).updateData({
            'location':LocationName,
            'category': modifyName,
          });
        }

        var aka = await Firestore.instance.collection('TempItemCollectionHold').document(LocationName + name).collection('item').getDocuments();

        for(int i = 0; i < aka.documents.length; i++){
          var data = aka.documents[i].data;
          await Firestore.instance.collection('TempItemCollectionHold').document(LocationName + modifyName).collection('item').document().setData(data);
          await Firestore.instance.collection('TempItemCollectionHold').document(LocationName + name).collection('item').document(aka.documents[i].documentID).delete();
        }
        await Firestore.instance.collection('TempItemCollectionHold').document(LocationName + name).delete();
        
        
        pop_window(langaugeSetFunc('Succeed'), langaugeSetFunc("Edit the category Successfully"), context);
      }
    }
    ...
    if(globals.isiOS){
      return Scaffold(
        appBar: CupertinoNavigationBar(
          heroTag: "tab311010Manage category",
          transitionBetweenRoutes: false,
          middle:  Text(langaugeSetFunc('Manage category'), style: TextStyle(color: textcolor()),),
