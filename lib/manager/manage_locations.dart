  ...
  void _showDialog1(name, imageURL, documentID) {
    ...
    void submit() async {
      ...
      if (form.validate()) {
        ...
        var  a = await Firestore.instance.collection(returnItemCollection()).where('Location', isEqualTo: LocationName).getDocuments();
        var b = await Firestore.instance.collection(returnReservationCollection()).where('location', isEqualTo: LocationName).getDocuments();
        ...
        await Firestore.instance.collection(returnLocationsCollection()).document(documentID).get().then((value){
          var categoryList = value.data["categories"].toList();
        ...
        for(int i = 0; i < listcater.length; i++){
          var categoryName = listcater[i]['name'];
          try{
            var a = await Firestore.instance.collection('TempItemCollectionHold').document(name + categoryName).collection('item').getDocuments();
            for(int i = 0; i < a.documents.length; i++){
              var data = a.documents[i].data;
              print(data);
              await Firestore.instance.collection('TempItemCollectionHold').document(name + categoryName).collection('item').document(a.documents[i].documentID).delete();
              await Firestore.instance.collection('TempItemCollectionHold').document(modifyName + categoryName).collection('item').document().setData(data);
            }
            await Firestore.instance.collection('TempItemCollectionHold').document(name + categoryName).delete();
          }catch(e){
            print(e);
          }
        }
    ...
    if(globals.isiOS){
      return Scaffold(
        appBar: CupertinoNavigationBar(
          ...
          middle:  Text(langaugeSetFunc('Manage Locations'), style: TextStyle(color: textcolor()),),
